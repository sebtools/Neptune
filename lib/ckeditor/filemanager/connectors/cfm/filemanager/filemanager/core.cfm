...
--->
<cfset Variables.FileMgr = Application.FileMgr>
...
<cffunction name="rename" access="public" output="false" returntype="string">
	<cfscript>
		...
		loc.fileLocation = Root & arguments.old;
		
	
		
		if (DirectoryExists(loc.fileLocation))
		{
			loc.newLocation = Root & loc.newPath & "/";
...
<cffunction name="add" access="public" output="false" returntype="string">
	<cfscript>
		...
		if (FileExists(loc.fileDestination)) {
			loc.fileDestination = Variables.FileMgr.createUniqueFileName(loc.fileDestination);
		}
