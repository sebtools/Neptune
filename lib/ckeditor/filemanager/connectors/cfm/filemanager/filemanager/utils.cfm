...
<cffunction name="$relativePathFromRoot" access="private" output="false" returntype="string">
	...
	<cfset var Root = $getRoot()>
	<cfset var RelativePath = arguments.path>
	
	<cfif Len(Root)>
		<cfset RelativePath = ReplaceList(Replace(arguments.path, Root, "", "all"), "\", "/")>
	</cfif>
...
<cffunction name="$thumbLocation" access="private" output="false" returntype="string">
	...
	<cfreturn Replace(arguments.path, ListLast(arguments.path,"/"), config.images.thumbnailFolder & "/" & ListLast(arguments.path,"/"), "one") />
</cffunction>
