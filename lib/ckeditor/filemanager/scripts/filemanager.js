...
var selectItem = function(data){
	...
	if(relPath !== false) {
		url = relPath + data['Path'].replace(fileRoot,""); 
	} else {
		url = relPath + data['Path'];
	}
...
var getFileInfo = function(file){
	...
	$.getJSON(fileConnector + '?mode=getinfo&path=' + encodeURIComponent(file) + '&time=' + d.getMilliseconds(), function(data){
		if(data['Code'] == 0){
			data['Filename'] = data['Filename'] + ''; // Handles filenames that are numeric
		...
		if(data){
			if($('#fileinfo').data('view') == 'grid'){
				...
				for(key in data){
					...
					if(typeof props !== 'undefined' && props['Width'] && props['Width'] != ''){
						result += ('<td>' + props['Width'] + 'x' + props['Height'] + '</td>');
					} else {
						result += '<td></td>';
					}
					
					if(typeof props !== 'undefined' && props['Size'] && props['Size'] != ''){
						result += '<td><abbr title="' + props['Size'] + '">' + formatBytes(props['Size']) + '</abbr></td>';
					} else {
						result += '<td></td>';
					}
					
					if(typeof props !== 'undefined' && props['Date Modified'] && props['Date Modified'] != ''){
...
/*	if(!config.options.fileRoot) {
	...
	fileRoot = fmHelpers.getFileRoot();
	relPath = decodeURIComponent(fmHelpers.getQueryVariable('relpath'));
	...
	if(relPath === false || relPath == 'false') {
		relPath = window.location.protocol + "//" + window.location.host;
	}
