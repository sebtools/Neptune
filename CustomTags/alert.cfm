...
<cfif ThisTag.executionMode IS "End" OR NOT thisTag.hasEndTag>
	...
	<cfif StructKeyExists(Variables,"Mailer")>
		...
		<!--- Make sure we have a nessage (as we need a subject) --->
		<cfif NOT Len(Trim(Attributes.message))>
			<cfset Attributes.message = "Alert">
		</cfif>
		<!--- Make sure we have an extended_message (as we need an email body) --->
		<cfif NOT Len(Trim(Attributes.extended_message))>
			<cfset Attributes.extended_message = Attributes.message>
		</cfif>
